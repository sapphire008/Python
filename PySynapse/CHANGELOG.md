...
* Recreated a conda environment (`environment.yml`) and documented setup.
* Startup paths moved from hardcoded `SynapseQt.py` branches into dynaconf `settings.yaml`.
* PyQt5 compatibility: `setSectionResizeMode`, widgets under `QtWidgets`, `QPoint` ints, Ctrl+C quits the event loop.
* `theme = whiteboard` in `resources/config.ini` now applies a light Fusion palette (macOS dark mode no longer takes over the chrome).
* File → Load Database CSV layout documented; finder script writes `database/persistent_activity.csv`.
* `get_cellpath` accepts hyphens in cell names (e.g. `Neocortex B Layer 2-3`).
...
* Added settings window. Default settings are saved under ./resources/config.ini and interfaced by GUI under the main synapse window: File/Settings.
